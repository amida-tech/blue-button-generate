...
var gen = function (data, CCD, xmlDoc, section_name) {
    if (data) {
        if (section_name === "demographics") {
            ...
            js2xml.fillUsingTemplate(xmlDoc, data, sectionLevel.payersSectionEntriesRequired);
...
var genWholeCCDA = function (data) {
    ...
    if (count_sections > 0) {
        ...
        for (var i = 2; i < Object.keys(sectionNames).length; i++) {
            if ((sectionNames[i] === 'payers') || (sectionNames[i] === 'plan_of_care') || (sectionNames[i] === 'procedures') || (sectionNames[i] === 'encounters') || (sectionNames[i] === 'allergies') || (sectionNames[i] === 'medications') || (sectionNames[i] === 'immunizations') || (sectionNames[i] === 'problems')) {
