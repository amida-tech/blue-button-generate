...
var fillAttributesUsingSpec = function (node, input, template) {
    var attrSpec = template.attributes;
    var inputKey = template.attributeKey;
    if (inputKey) {
...
var fillSingleUsingTemplate = function fillSingleUsingTemplate(xmlDoc, input, name, template) {
    ...
    if ((!condition) || condition(input)) {
        ...
        fillAttributesUsingSpec(node, input, template);
