...
var code = function code(xmlDoc, entity, tag) {
    ...
    if (entity) {
        ...
        if (entity.code_system_name && (entity.code_system_name.substring(0, 4) === 'OID ')) {
            ...
            var code_system_attr = entity.code_system || (entity.code_system_name && codeSystems[entity.code_system_name] && codeSystems[entity.code_system_name][0]);
            if (code_system_attr) {
                c.attr({
...
var header = function (doc, templateIdOptional, templateIdRequired, code, codeSystem, codeSystemName, displayName, title, isCCD) {
    ...
    asIsCode(xmlDoc, {
        ...
        code_system_name: codeSystemName
    });
...
var effectiveTime = function (xmlDoc, rawTime, tag, xsi_type) {
    var time = getTimes(rawTime);
...
var indicationConstraint = function (xmlDoc, findings) {
    if (findings) {
        ...
        if (findings[0].date_time) {
            effectiveTime(ob, findings[0].date_time);
        }
