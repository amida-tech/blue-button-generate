...
var updateSupply = function (xmlDoc, entry, index) {
    ...
    if (entry.supply) {
        if (entry.supply.date_time) {
            libCCDAGen.effectiveTime(s, entry.supply.date_time, undefined, "IVL_TS");
        }
        ...
        if (entry.supply.author) {
            ...
            if (entry.supply.author.date_time) {
                libCCDAGen.effectiveTime(a, entry.supply.author.date_time, 'time');
            } else {
                a.node('time').attr({
                    nullFlavor: "UNK"
                });
...
var updateDispense = function (xmlDoc, dispense, index) { // this time needs to be investigated
    if (dispense) {
        var er = xmlDoc.node('entryRelationship').attr({
            typeCode: "REFR"
        }); // Medication Dispense
        ...
        if (dispense.date_time) {
            libCCDAGen.effectiveTime(s, dispense.date_time);
        }
        if (dispense.product) {
...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        updateDispense(sa, data[i].dispense, i);
