...
var updateStatus = function (xmlDoc, entry, index) {
    ...
    if (entry.status) {
        if (entry.status.date_time) {
            libCCDAGen.effectiveTime(ob, entry.status.date_time);
        }
        var fullCode = libCCDAGen.reverseTable("2.16.840.1.113883.3.88.12.80.68", entry.status.name);
        fullCode.code_system = "2.16.840.1.113883.6.96";
        fullCode.code_system_name = "SNOMED CT";
        libCCDAGen.value(ob, fullCode, "CD");
    }
...
module.exports = function (data, codeSystems, isCCD, CCDxml) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        libCCDAGen.effectiveTime(act, data[i].date_time);
        ...
        if (data[i].problem) {
            libCCDAGen.effectiveTime(ob, data[i].problem.date_time);
